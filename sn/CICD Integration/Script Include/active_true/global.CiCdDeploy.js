 ...
 * Created On  : 2018-08-13 12:12:30
 * Created By  : b.moers
 * Updated On  : 2019-11-26 19:54:14
 * Updated By  : admin
 * URL         : /sys_script_include.do?sys_id=79e4545fdbbb134076d6b94ffe9619bf
 */
/* exported CiCdDeploy */
...
CiCdDeploy.prototype = /** @lends global.module:sys_script_include.CiCdDeploy.prototype */ {
	...
	gs.info('Step 1. Inside deploy updateset function');
        /*
            {
        ...
        */
        ...
        try {
			...
			gs.info('Step 1a. Commit Id...'+commitId);
            updateSetSysId = self.body.updateSetSysId; // request.updateSetId
			gs.info('Step 2. update set sys id..... '+updateSetSysId);
			...
			gs.info('Step 3. Source URL.... '+sourceUrl);
            targetEnvironment = self.body.targetEnvironment.host.toLowerCase().replace(/\/$/, "");
			gs.info('Step 4. Target environment..... '+targetEnvironment);
            ...
            if (gitDeployment) { // git 2 snow deployment
				gs.info('Step 5. Inside git deployment condition...');
                if (!updateSetSysId) { // if no updateSetId provided, derive from the commit id
					...
					gs.info('Step . Inside git deployment condition...'+updateSetSysId);
                }
                limitSet.push(commitId);

            } else { // for snow 2 snow deployment, check us state etc
                var us = new GlideRecord('sys_update_set');
				//updateSetSysId='89f3de66db914010343f58b3ca961921';
                ...
                */
                if (us.base_update_set.nil()) {
					gs.info('Step 6. Inside base update empty deployment condition...');
                    limitSet.push(updateSetSysId);
                } else {
					var baseSysId = us.getValue('base_update_set');
					gs.info('Step 7 . Inside base update deployment condition...'+baseSysId);
                    ...
                    bus._query();
                    while (bus._next()) {
						gs.info('Step 8 . Inside base update deployment condition...'+bus.getValue('sys_id'));
            ...
            if (!response.haveError()) {
                ...
                */
				...
				gs.info(responseBody+'Step 9 . limet set - Rest API response...'+limitSet);
                var payload = {
    ...
    _targetLoadUpdateSet: function (payload) {
        var self = this;
gs.info('Step 10 . inside targe load updateset....'+payload);
...
gs.info('Step 11 . limet set - target load...'+limitSet);
